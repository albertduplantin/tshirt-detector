...
// Fonction d'analyse des lunettes
const analyzeGlasses = (data: Uint8ClampedArray, width: number, area: any) => {
  let reflectionPixels = 0
  let totalPixels = 0
  let edgeCount = 0

  // Zone spécifique pour les lunettes (centre du visage)
  const glassArea = {
    startX: area.startX + Math.floor((area.endX - area.startX) * 0.1),
    endX: area.endX - Math.floor((area.endX - area.startX) * 0.1),
    startY: area.startY + Math.floor((area.endY - area.startY) * 0.3),
    endY: area.startY + Math.floor((area.endY - area.startY) * 0.7)
  }

  for (let y = glassArea.startY; y < glassArea.endY; y++) {
    for (let x = glassArea.startX; x < glassArea.endX; x++) {
      const index = (y * width + x) * 4
      const r = data[index]
      const g = data[index + 1]
      const b = data[index + 2]
      ...
      // Détecter les reflets (très lumineux)
      if (brightness > 200 && Math.abs(r - g) < 30 && Math.abs(g - b) < 30) {
        reflectionPixels++
      }
      
      // Détecter les montures (contrastes élevés)
  ...
  if (reflectionRatio > 0.05 && edgeRatio > 0.1) {
    type = 'Lunettes de vue'
    confidence = Math.min(0.9, 0.5 + reflectionRatio * 2 + edgeRatio * 2)
  } else if (reflectionRatio > 0.08) {
    type = 'Lunettes de soleil'
    confidence = Math.min(0.85, 0.4 + reflectionRatio * 3)
  } else if (edgeRatio > 0.15) {
    type = 'Monture sans verres'
    confidence = Math.min(0.7, 0.3 + edgeRatio * 2)
  }
